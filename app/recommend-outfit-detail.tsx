...
export default function RecommendOutfitDetailScreen() {
  ...
  const generateTryOn = async () => {
    ...
    try {
      // 🌟 1. 백엔드에서 생성된 아바타 URL(http://...)을 우선하여 읽어옵니다.
      let finalAvatarPath = (await AsyncStorage.getItem('USER_AVATAR_URL'))
        || (await AsyncStorage.getItem('USER_AVATAR_IMAGE'));

      // 🌟 2. 혹시라도 AsyncStorage에 폰 로컬 주소(file:// 또는 var/mobile)가 들어있다면 예외 방어
      if (!finalAvatarPath || finalAvatarPath.includes('var/mobile') || finalAvatarPath.startsWith('file:')) {
        console.warn('⚠️ 생성된 아바타 URL을 찾을 수 없어 백엔드 기본 아바타를 요청합니다.');
        finalAvatarPath = `${API_BASE_URL}/output/base_avatar.png`;
      }

      if (!current) {
        console.log('❌ 코디 정보가 없음');
        return;
      }

      setFaceImage(finalAvatarPath);
      ...
      console.log('🔥 아바타 경로:', finalAvatarPath);
      ...
      console.log('✅ 옷 입히기 생성 결과:', response.data);
