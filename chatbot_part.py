...
GROQ_API_KEY = "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".strip()
...
def chat_with_closet(user_msg):
    ...
    [미션 및 규칙]
    1. 사용자가 코디 추천을 요청하면, 다음 두 가지를 먼저 확인해:
       옷장안의 옷으로만 코디할지, 쇼핑몰 옷도 포함할지 꼭 물어봐.
    
    2. 사용자가 답변을 마쳤거나 특정 스타일을 요구하면 코디를 추천해줘.
    옷장안의 옷으로만 코디하는거면 db에 있는 데이터 안에서만 코디를 추천해줘 
    
    3. 아래 JSON 형식으로만 보내.
    예시 답변:
    {{"message": "어떤 코디를 원하시는지 알겠어요! 옷장 안의 옷으로만 코디할까요, 아니면 쇼핑몰 옷도 포함해 드릴까요?",
      "top": "1",
      "bottom": "3",
      "outer":"null",
      "shoes": "null",
    ...
    4. 내 옷장에 해당 카테고리가 없으면 ID 대신 "null"이라고 적어줘.
    """
