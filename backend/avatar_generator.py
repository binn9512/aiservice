...
def generate_avatar_image(photo):
    ...
    try:
        ...
        print("🔥 Replicate 원본 응답 타입:", type(output))
        print("🔥 Replicate 원본 응답 값:", output)

        # =========================================================
        # 🌟 Replicate FileOutput 및 다양한 반환 타입 완벽 대응
        # =========================================================
        output_url = None
        ...
        # 2. FileOutput 객체 또는 url 속성을 가지고 있는 경우
        if hasattr(output, "url"):
            output_url = str(output.url)
        elif isinstance(output, str):
            output_url = output
        else:
            # FileOutput 객체 자체를 string으로 바꾸기 전 처리
            output_url = str(output)

        # HTTP URL 형식인지 검증
        if not output_url or not output_url.startswith("http"):
            raise ValueError(
                f"올바른 이미지 URL을 받아오지 못했습니다. (받은 값: {output_url})"
            )
