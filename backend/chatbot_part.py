...
def chat_with_closet(user_msg, room_id="default"):
    ...
    상황 1. 사용자가 [원하는 스타일/용도]만 말하고, 아직 쇼핑몰 포함 여부는 밝히지 않은 경우 (예: "하객룩 추천해줘", "데이트룩 짜줘"):
        - 절대로 바로 코디를 추천하지 마라!
        - 사용자가 원하시는 스타일을 확인했음을 다정하게 알리면서, **"내 옷장에 있는 옷으로만 추천해 드릴까요, 아니면 쇼핑몰 옷도 함께 추천해 드릴까요?"** 라고 출처 선택 질문만 물어봐라.
    ...
    상황 2. 사용자가 [쇼핑몰 옷 포함 여부]를 대답했거나 요구사항이 명확한 경우 (예: "내 옷장만 써줘", "쇼핑몰 옷도 포함해줘", "하의만 쇼핑몰로"):
        ...
        - "내 옷장만" ➡️ MY_ ID로만 구성
        - "쇼핑몰 포함/쇼핑몰 옷 추천" ➡️ MY_ 와 SHOP_ ID를 적절히 조합하거나 SHOP_ 위주로 구성, 하지만 모든 옷 다 SHOP_ 인것은 가능하면 피하기.
    ...
    상황 4. 수정 요청 처리 규칙:
        - 사용자가 '특정 아이템'을 바꿔달라고 하면(예: 상의만 바꿔줘, 바지는 쇼핑몰 옷으로 바꿔줘), 기존 추천했던 다른 아이템 ID는 유지하고 해당 항목만 변경해라.
