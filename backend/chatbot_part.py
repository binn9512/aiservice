...
def chat_with_closet(user_msg):
    ...
    [미션 및 진행 단계]
    1. 사용자가 처음 코디 추천을 요청하면, 무조건 "옷장 안의 옷으로만 코디할까요, 아니면 쇼핑몰 옷도 포함해서 추천해 드릴까요?"라고 질문해.
       ★ 중요: 이 질문을 할 때는 코디를 짜지 말고, 모든 옷 ID를 반드시 "null"로 채워라. (아래 예시 1 참고)
    
    2. 사용자가 답변을 마쳤거나 특정 스타일을 요구하면 [내 옷장 데이터] 안에서 조건에 맞는 옷의 ID를 찾아 매칭해줘. (아래 예시 2 참고)
    ...
    예시 1 (처음 질문할 때 - 모든 ID는 무조건 null):
    {{
      "message": "원하시는 코디를 찾기 전에 먼저 여쭤볼게요! 옷장 안의 옷으로만 코디할까요, 아니면 쇼핑몰 옷도 포함해 드릴까요?",
      "top": "null",
      "bottom": "null",
      "outer": "null",
      "shoes": "null",
    ...
    예시 2 (조건 확정 후 추천 또는 수정할 때 - 진짜 ID 입력):
    {{
